...
If the plan includes 立减金 or any `benefit_type=price_discount`, use:

`final payment price = base price - discount_amount`

`expected GMV = expected sales * final payment price`

Do not use the pre-discount base price for GMV when 立减金 is included.
...
- A plan uses 立减金 but does not clearly show final payment price.
- A plan uses 京东卡 and incorrectly treats it as payment-price reduction.
